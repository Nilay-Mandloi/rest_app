...
class ArtifactManifest(BaseModel):
    """Immutable per-version metadata written next to model.pkl in S3.

    Field names and required/optional split must stay in sync with the mlops
    dataclass — verified by tests/test_contract_sync.py::test_python_class_field_parity.
    """
...
class PointerFile(BaseModel):
    """Mutable per-model channel pointer (stable.json / latest.json).
